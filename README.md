...
- 地図アイコンは地域別に色分けしません。ケーキ、カフェ、デパ地下の用途別に統一します。
- スマホで重くならないよう、初期マーカーと一覧描画は必要量に制限します。
...
- デパ地下は個別ブランドを無制限に増やすのではなく、まず百貨店/駅ビルのスイーツ売場ハブとして追加する
- デパ地下ハブは `type: ["depachika", "gift", "walk"]` を基本にし、特に強い売場だけ `top` を足す

## デパ地下内リスト

デパ地下ハブの中で見るべき代表ブランドは `depachikaItems` に持たせます。

- 店舗本体の位置情報と、中に入っている代表ブランド一覧を分ける
- デパ地下フィルタを選んだ時は、カード内に「中で見るもの・代表例」を表示する
- ポップアップにも同じ代表例を表示する
- 百貨店側の入れ替えがあるので、断定ではなく代表例として扱う

## 写真表示

店舗データに `image` がある場合だけ、カードとポップアップに写真を表示します。

写真追加の推奨ルール:

- 公式サイトのOG画像、公式が配布/掲載している画像、許諾済み画像、自前で撮影した画像、ライセンス確認済み画像だけを使う
- 食べログ、Google Map、Instagram投稿写真の無断直リンクは避ける
- 画像には `alt`、可能なら `credit` と `source` を入れる
- 直リンクが不安定な場合は、許諾を確認したうえでリポジトリ内に保存する
- 写真がない店舗は無理に汎用画像を出さない。代表性の低い画像は地図の品質を下げる
...
スマホ表示が重くなりやすいので、以下の制御を入れています。

- 初期状態で全191件のマーカーを一度に作らない
- `activeType === "all"` かつ件数が多い場合、地図には `top` と `depachika` を中心に最大80件だけ出す
- 一覧は最初30件だけ描画し、`さらに表示` で30件ずつ増やす
- 一覧から未描画マーカーの店を開いた場合、その店のマーカーだけ遅延作成する
- 検索入力は即時再描画ではなく、短い待ち時間を置いて反映する

安易に `filtered.forEach` で全マーカー/全カードを毎回生成する実装へ戻さないでください。
...
今後は北関東、神奈川郊外、埼玉郊外、千葉郊外が薄いので、そこを増やす余地があります。
